...
# Financial flows analysis: Stage 4 -- Mapping deal participants ulitmate parents -------
# github.com/lyd-m/amazon-footprint
# This project contains the financial analysis for the paper, Singh et al. (2025)
# This script takes a set of deals pulled for companies linked to the Amazon and maps the ultimate parents of the entities involved in the deal.
...
for asset_class in asset_classes_flows:
    ...
    ups_to_search = [
        i for i in ups if i not in ultimate_parents_database["query"].values
    ]
    print(f"Searching {len(ups_to_search)} new queries for ultimate parents data")

    # ups_to_search = ups #version without filtering ones searched before
            ...
            try:
                ...
                if search_result.empty:
                    search_result = pd.DataFrame(
                        {"query": [search_query], "source": [asset_class]}
                    )
...
# tidy up output
ups_df = ups_df.drop("source", axis=1)
cols = ["query"] + [col for col in ups_df.columns if col != "query"]
ups_df = ups_df[cols]

ups_df.to_csv(
    f"./intermediate-results/{today}_ups_new_raw.csv", index=False
...
ups_df["query_clean"] = ups_df["query"].apply(clean_text)
ups_df["CommonName_clean"] = ups_df["CommonName"].apply(clean_text)


# function to compute similarity
def fuzzy_match(row):
    return round(fuzz.ratio(row["query"], row["CommonName"]), 3)  # returns similarity %
...
to_check = ups_df[ups_df["manual_check_needed"] == "TRUE"]

with pd.ExcelWriter(
    f"./intermediate-results/ups-manual-checking/{today}_ups_checking.xlsx",
    engine="openpyxl",
) as writer:
    to_check.to_excel(writer, sheet_name="to_check", index=False)
    matched_ups.to_excel(writer, sheet_name="no_check", index=False)
...
# overwrite master ultimate parents database with new data that doesn't require checking
